...
public class C01_Dropdown {
    ...
    //        3. Select object i kullaarak 3 farkli sekilde secim yapabilirim
        yearDropdown.selectByIndex(32);//SECENEK SIRASI 0 DAN BASLAR. 1991 yili 33. sirada
    //        Dogun yilini, ayini, ve gununu su sekilde secer : 1991, August, 15
    //      Ay Secimi
        WebElement ay = driver.findElement(By.xpath("//select[@id='month']"));
        Select ayDropdown = new Select(ay);
        ayDropdown.selectByValue("7");//option in value degeri ile secim yapilabilir. August seceneinin value si 0
    //        Gun secimi selectByVisibleTest
        WebElement gun = driver.findElement(By.id("day"));
        Select gunDropdown = new Select(gun);
        gunDropdown.selectByVisibleText("15");//CASE SENSITIVE -(buyuk kucuk harf onemli)
    }
    @Test
